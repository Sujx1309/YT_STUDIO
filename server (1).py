...
def clean_youtube_url(url):
    """
    YouTube na badha URL formats ne standard format ma convert karo.
    youtu.be, ?si=, ?list= with start_radio, m.youtube.com, music.youtube.com
    """
    try:
        ...
        if host in ("youtube.com", "music.youtube.com"):
            ...
            vid = qs.get("v", [None])[0]
            if vid:
                # Playlist download nathi karta? Sirf video ID rakho
                return f"https://www.youtube.com/watch?v={vid}"
            # /shorts/VIDEO_ID
            if "/shorts/" in p.path:
        ...
        # koi bijo platform hoy to as-is return karo
        return url
    except Exception:
        return url
...
def download_video(job_id, url, opts):
    ...
    try:
        ...
        for line in proc.stdout:
            ...
            if m2:
                full_path = m2.group(1).strip()
                progress_store[job_id]["filename"] = os.path.basename(full_path)
                progress_store[job_id]["filepath"] = full_path
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/index.html":
            ...
            self.send_static_file(path[1:], "image/png")  # strip leading /
            ...
            if not job or job.get("status") != "done":
                self.send_response(404)
                self.end_headers()
                self.wfile.write(b"File taiyar nathi ya job malyo nahi")
                return
            ...
            if not filepath or not os.path.exists(filepath):
                self.send_response(404)
                self.end_headers()
                self.wfile.write(b"File disk par malyi nahi")
                return
            ext = os.path.splitext(filepath)[1].lower()
            mime_map = {".mp4": "video/mp4", ".mp3": "audio/mpeg", ".m4a": "audio/mp4",
                        ".webm": "video/webm", ".mkv": "video/x-matroska", ".opus": "audio/opus"}
            mime = mime_map.get(ext, "application/octet-stream")
            fname_safe = os.path.basename(filepath)
            file_size = os.path.getsize(filepath)
            self.send_response(200)
            self.send_header("Content-Type", mime)
            self.send_header("Content-Length", file_size)
            self.send_header("Access-Control-Allow-Origin", "*")
            self.send_header("Content-Disposition", f'attachment; filename="{fname_safe}"')
            self.end_headers()
            try:
